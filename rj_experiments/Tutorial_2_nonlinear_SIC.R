library(SICNN)
#### Tutorial 2 (SIC): simulated data with non-linear effects using smooth BIC criterion
...
y_base <- -3 + 0.1 * log(abs(X[, 1])) + 3 * cos(X[, 2]) +
  2 * X[, 3] * X[, 4] + X[, 5] - X[, 6]^2 + rnorm(i, sd = 0.1)

y <- numeric(length(y_base))
